...
if (selectedOption) {
  selectedBranchCode = selectedOption.key;
   //item10.push(selectedOption.text as string);
   //this.setState({BranchCode: []});
   console.log(this.state.BranchCode);
   sp.web.lists.getByTitle("BranchDetails").items.select('Title,BranchName, Zone, Region').filter("Title eq '" + selectedBranchCode + "'").get().then(function(data){
    for(var k in data){
      BranchCode.length=0;
      zone.length=0;
      region.length=0;
     ...
     region.push({key:data[k].Region, text:data[k].Region});
      ...
      reacthandler.setState({BranchCode});
      reacthandler.setState({zone});
      reacthandler.setState({region});
   ...
   //item9.push(BranchCode);
   //this.setState({selectedBranchCode: item9})
